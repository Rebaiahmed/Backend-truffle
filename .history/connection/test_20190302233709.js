...
self.web3.eth.defaultAccount=self.web3.eth.coinbase;
  ...
  Food.deployed().then(function(instance) {
    ...
    console.log("data returend" +JSON.stringify(data.receipt.logs));
    //callback("success");
...
return food.addFarmer("data mte3o hethi",{from:self.web3.eth.defaultAccount,
  gas:3000000 });

}).then(function(data2) {
  console.log("data returend" +JSON.stringify(data2.receipt.logs));
  //callback("success");
  ...
  console.log(e);
  //callback("ERROR 404");
});//____end catch addFarmer___________/:
