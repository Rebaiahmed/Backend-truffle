...
self.web3.eth.defaultAccount=self.web3.eth.coinbase;
  ...
  Food.deployed().then(function(instance) {
    ...
    //console.log('instance is' + food.farmers[0]);
     ...
     return food.getCompanyById(0,0,{from:self.web3.eth.defaultAccount,
      gas:3000000 });


  }).then(function(data) {

     console.log('current product is ' + data[9]);
...
return food.getCompanyById(2,0,{from:self.web3.eth.defaultAccount,
  gas:3000000 })
  .then(function(data2){

console.log('the farmer is' + JSON.stringify(data2));
   callback(Number(data)+1);
  }).catch(function(err){
    console.log('err searching farmer' + JSON.stringify(err));
  })
